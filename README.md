[README_architecture_reference.md](https://github.com/user-attachments/files/28755175/README_architecture_reference.md)
...
#### Test wytrzymałości (endurance / soak)

Cel: sprawdzić stabilność systemu w czasie pod stałym, umiarkowanym obciążeniem
(czy nie rośnie opóźnienie, nie pojawiają się błędy, nie ma wycieków). 30 równoległych
użytkowników, przyrost 5/s, czas trwania 300 s:

```bash
locust -f tests/performance/locustfile.py --host http://localhost:8000 \
  --headless -u 30 -r 5 -t 300s --csv endurance --only-summary
```

| Endpoint | # req | Fails | Avg [ms] | Med [ms] | p95 [ms] | Max [ms] | req/s |
|---|---|---|---|---|---|---|---|
| `GET /api/ranking?genre=metal` | 1839 | 0 | 65 | 68 | 120 | 172 | 6,16 |
| `GET /api/cities` | 952 | 0 | 57 | 60 | 110 | 162 | 3,19 |
| `GET /api/cities/1/events?genre=metal` | 991 | 0 | 81 | 85 | 140 | 192 | 3,32 |
| `GET /api/dashboard/1?genre=metal` | 469 | 0 | 275 | 270 | 430 | 523 | 1,57 |
| Aggregated | 4251 | 0 (0,00%) | 90 | 72 | 280 | 523 | 14,23 |

Wnioski: przez całe 5 minut zero błędów i ok. 14,2 req/s (4251 żądań). Czasy odpowiedzi
pozostały stabilne i zbieżne z testem 50-użytkownikowym. Opóźnienia nie narastały w czasie,
co potwierdza brak degradacji i wycieków przy ciągłej pracy. Najwolniejszy pozostaje
`/api/dashboard` (mediana 270 ms), bo agreguje dane z kilku serwisów.

#### Test stresowy (stress)

Cel: znaleźć granicę wydajności 
i zobaczyć, gdzie system zaczyna się dławić. 300 równoległych użytkowników, przyrost 30/s,
czas trwania 120 s:

```bash
locust -f tests/performance/locustfile.py --host http://localhost:8000 \
  --headless -u 300 -r 30 -t 120s --csv stress --only-summary
```

| Endpoint | # req | Fails | Avg [ms] | Med [ms] | p95 [ms] | Max [ms] | req/s |
|---|---|---|---|---|---|---|---|
| `GET /api/ranking?genre=metal` | 3595 | 0 | 1588 | 1600 | 2100 | 7521 | 30,18 |
| `GET /api/cities` | 1767 | 0 | 1583 | 1600 | 2100 | 7380 | 14,83 |
| `GET /api/cities/1/events?genre=metal` | 1859 | 0 | 1602 | 1600 | 2100 | 6588 | 15,61 |
| `GET /api/dashboard/1?genre=metal` | 907 | 0 | 7192 | 7400 | 8900 | 10862 | 7,61 |
| Aggregated | 8128 | 0 (0,00%) | 2215 | 1600 | 7500 | 10862 | 68,24 |

Wnioski: nawet przy 6-krotnie wyższym obciążeniu (300 użytkowników) gateway obsłużył 8128
żądań bez ani jednego błędu HTTP i osiągnął najwyższą przepustowość ok. 68 req/s. System
nie przewrócił się, lecz nasycił (graceful degradation). Widać wyraźną granicę
wydajności: czasy odpowiedzi wzrosły kilkunastokrotnie (mediana zwykłych endpointów ~1,6 s),
a wąskim gardłem stał się `/api/dashboard`, średnio 7,2 s, maks. ~10,9 s, ponieważ każde
jego wywołanie odpytuje sekwencyjnie kilka serwisów wewnętrznych. To naturalny kandydat do
optymalizacji (cache wyników, równoległe zapytania, więcej workerów uvicorn).

**Podsumowanie:** poziom roboczy (≈30–50 użytkowników) jest obsługiwany z czasami poniżej
~300 ms i bez błędów; granica komfortu leży znacznie niżej niż próg awarii, przy 300
użytkownikach system nadal działa poprawnie, jedynie wolniej.
